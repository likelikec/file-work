...
# FEATURES =['Leve+l', 'Dc+y', 'Cyclic', 'jf', 'COM_RAT',
...
# #sym
# FEATURES=['Leve+l', 'Dc+y', 'Cyclic', 'jf', 'COM_RAT', 'DP+T', 'INNER', 'JLOC', 'Level', 'CLOC', 'Jm', 'PDpt', 'LCOM' ,
#           "String processing", "Mathematical calculation", "User Interface", "Business Logic",  "Exception handling"]
# #交集
# FEATURES=[ 'Dc+y', 'Cyclic', 'jf', 'COM_RAT', 'DP+T', 'INNER',
#            'Leve+l','Level' ,'CLOC',  'PDpt', 'Jm','LCOM' ,"String processing", "Business Logic"]
def clean_data(df):
    """数据清洗函数"""
    df_clean = df.copy()
...
def main():
    try:
        # 数据加载与清洗
        # df = pd.read_excel(r"C:\Users\17958\Desktop\train2.0_processed.xlsx")
        ...
        # original_size1 = len(df1)
        # original_size2 = len(df2)
        df_clean1 = clean_data(df1)
        df_clean2 = clean_data(df2)

        # print(f"数据清洗完成，原始数据量: {original_size1,original_size2}，处理后数据量: {len(df_clean1,df_clean2)}")
